...
async def editing_profil(callback : types.CallbackQuery, state : FSMContext):
    ...
    if value in ('name', 'age', 'country', 'city', 'indy'):
        ...
        await callback.message.answer(f'❗️Введи новое {dict[value]} для вашего профиля')
    ...
    async with state.proxy() as data:
        ...
        if value == 'age':
            if message.text.isdigit():
                if int(message.text) in range(14, 99):
                    await update_profil(data['value'], message.text, message.chat.id)
                    await message.answer('Профиль успешно отредактирован 💁🏼‍♀️')
...
async def save_for_callback(callback : types.CallbackQuery, state : FSMContext):
    ...
    async with state.proxy() as data:
        ...
        await callback.message.delete()
        await callback.message.answer('Профиль успешно отредактирован 💁🏼‍♀️')
...
async def edit_disease_choise_1(callback : types.CallbackQuery, state : FSMContext):
    async with state.proxy() as data:
        ...
        if callback.data.split('_')[1] == 'Зависимость':
            ...
            await send_disease_some_for_redact(callback, 'd')
...
async def save_for_callback(callback : types.CallbackQuery, state : FSMContext):
    ...
    if a == 'review':
        await callback.message.delete()
        await callback.message.answer('❗️ Сформулируйте в одном сообщениии ваше обращение')
